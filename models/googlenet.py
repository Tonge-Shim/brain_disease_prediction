...
class GoogLeNet(nn.Module):
    ...
    def __init__(
        ...
        init_weights: Optional[bool] = True,  ## default was None. changed to True to avoid warning.
        blocks: Optional[List[Callable[..., nn.Module]]] = None
    ) -> None:
        super(GoogLeNet, self).__init__()
        if blocks is None:
            blocks = [BasicConv3d, Inception, InceptionAux]
        ...
        self.conv1 = conv_block(n_input_channels, 64, kernel_size=7, stride=2, padding=3)  ## grayscale: input channel=1
...
class InceptionAux(nn.Module):
    ...
    def forward(self, x: Tensor) -> Tensor:
        # aux1: N x 512 x 14 x 14 x 14, aux2: N x 528 x 14 x 14 x 14
        x = F.adaptive_avg_pool3d(x, (4, 4, 4))
        # aux1: N x 512 x 4 x 4 x 4, aux2: N x 528 x 4 x 4 x 4
        x = self.conv(x)
        # N x 128 x 4 x 4 x 4
        x = torch.flatten(x, 1)
        # N x 8192
